...
### 4.正向代理
   * 一个位于客户端和原始服务器(origin server)之间的服务器，为了从原始服务器取得内容，客户端向代理发送一个请求并指定目标(原始服务器)，然后代理向原始服务器转交请求并将获得的内容返回给客户端。***客户端需要进行特别的设置才能使用正向代理***
   * ***感觉和反向代理异曲同工***
### 5.正向代理 VS 反向代理
   * eg：A、B、C三个人
      * 正向代理：A通过B向C借钱，A知道C的存在
      * 反向代理：A通过B向C借钱，A不知道C的存在，在A看来B就是债主，在C看来B就是借贷人
